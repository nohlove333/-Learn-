  ...
  function initFilePicker(dialog, initialFiles) {
    ...
    function paint() {
      ...
      var oldItems = (initialFiles || []).filter(function (file) { return keepIds.indexOf(file.id) >= 0; }).map(function (file) {
        return '<div class="selected-file"><span>기존 · ' + escapeHtml(file.name) + '</span>' +
          '<button type="button" data-remove-old="' + attr(file.id) + '">제거</button></div>';
      });
      var newItems = selected.map(function (file, index) {
        return '<div class="selected-file"><span>' + escapeHtml(file.name) + ' · ' + escapeHtml(fileSize(file.size)) + '</span>' +
  ...
  async function previewAttachment(file, role) {
    ...
    try {
      if (isImage) {
        ...
        stage.innerHTML = '<video controls playsinline src="' + attr(videoAccess.publicUrl || videoAccess.downloadUrl) + '"></video>';
      } else if (isPdf || isOffice) {
        var documentAccess = await temporaryAccess(file, role);
        stage.innerHTML = '<iframe src="' + attr(documentAccess.previewUrl) + '" title="' + attr(file.name) + ' 미리보기" allow="autoplay"></iframe>';
      } else {
        stage.innerHTML =
